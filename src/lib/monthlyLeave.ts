...
/** Batch version: monthly Holiday/Sick/Early-Leave counts for a list of users, for one month/year. */
export async function getMonthlyLeaveSummaryForUsers(
  ...
  for (const id of userIds) summary[id] = { holidaysTaken: 0, sickDays: 0, earlyLeaves: 0 }
  ...
  const [holidays, sicks, earlyLeaves] = await Promise.all([
