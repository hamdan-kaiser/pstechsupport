...
import { Clock, Upload, Download, Sun, Moon, Coffee, ChevronLeft, ChevronRight } from 'lucide-react'
import { cn, DAYS, DAY_LABELS, getWeekStart, shiftStyle, avatarColor, deriveRowStatus } from '@/lib/utils'
          ...
          <table className="w-full text-sm min-w-[700px]">
            ...
            <tbody>
              {entries.map((entry, i) => {
                ...
                // On the current week, show today's real status (Day/Night/Off). When browsing a
                // different week there's no "today" in view, so fall back to the assigned shift type.
                const todayValue = todayColIndex >= 0 ? entry[DAYS[todayColIndex]] : null
                const period = todayColIndex >= 0 ? deriveRowStatus(todayValue) : (entry.user?.shift === 'day' ? 'day' : 'night')
                return (
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1 mt-0.5">
                          {period === 'day'
                            ...
                            : <><Coffee className="w-3 h-3" style={{ color: 'var(--text-muted)' }} /><span className="text-xs" style={{ color: 'var(--text-muted)' }}>Off</span></>
                          }
                        </div>
                      </div>
                    </div>
